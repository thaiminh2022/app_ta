...
final class GameDatasource implements IGameDatasource {
  ...
  StringPreferencesEntry _board(String dictionaryKey, int index) =>
      StringPreferencesEntry(
        ...
        key: 'game.board.$dictionaryKey.$index',
      );

  BoolPreferencesEntry get _isFirstEnter => BoolPreferencesEntry(
    sharedPreferences: sharedPreferences,
    key: 'game.isFirstEnter',
  );
  ...
  Future<GameResult?> getDaily(String dictionary, String date) async {
    ...
    final board =
        decodedResult['board']
            ...
            .map(
              (e) =>
                  LetterInfo.fromJson(json.decode(e) as Map<String, dynamic>),
            )
            .toList();
  ...
  Future<void> setDailyBoard(
    ...
    String date,
    GameResult savedResult,
  ) async => _board(dictionary, GameMode.daily.index).setIfNullRemove(
    json.encode({
      'date': date,
