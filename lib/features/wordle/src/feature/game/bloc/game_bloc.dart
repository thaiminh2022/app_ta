...
import 'package:app_ta/features/wordle/src/feature/game/model/game_mode.dart';
import 'package:app_ta/features/wordle/src/feature/game/model/game_result.dart';
import 'package:app_ta/features/wordle/src/feature/game/model/keyboard.dart';
import 'package:app_ta/features/wordle/src/feature/game/model/letter_info.dart';
import 'package:app_ta/features/wordle/src/feature/game/model/word_error.dart';
...
final class GameBloc extends Bloc<GameEvent, GameState> {
  GameBloc({
    ...
    required GameRepository gameDataSource,
    required StatisticsRepository statisticsDataSource,
    required LevelRepository levelDataSource,
  ...
  Future<void> _changeDictionary(
    _GameChangeDictionary event,
    ...
    if (state.gameMode == GameMode.daily) {
      savedResult = await _gameRepository.getDaily(
        newDictionary,
        DateTime.now().toUtc(),
      );
    } else {
    ...
    final newState = _stateBySavedResult(
      ...
      _gameRepository.generateSecretWord(
        newDictionary,
        levelNumber:
            state.gameMode == GameMode.daily ? 0 : savedResult?.lvlNumber ?? 1,
      ),
    );
    emit(newState);
  }

  Future<void> _changeGameMode(
    _GameChangeGameMode event,
    Emitter<GameState> emit,
  ) async {
    ...
    final newState = _stateBySavedResult(
      ...
      _gameRepository.generateSecretWord(
        state.dictionary,
        levelNumber:
            newGameMode == GameMode.daily ? 0 : savedResult?.lvlNumber ?? 1,
      ),
    );
    emit(newState);
  }
  ...
  void _letterPressed(_GameLetterPressed event, Emitter<GameState> emit) {
    ...
    if (state.board.isNotEmpty &&
        ...
        state.board[state.currentWordIndex * 5].status ==
    ...
    emit(
      GameState.idle(
        ...
        board: List.of(state.board)
          ..add(LetterInfo(letter: event.key.toLowerCase())),
  ...
  void _enterPressed(_GameEnterPressed event, Emitter<GameState> emit) {
    ...
    if (word.join() == state.secretWord) {
      final correctWord = word.map(
        (e) => LetterInfo(letter: e, status: LetterStatus.correctSpot),
      );
      final newBoard = List.of(state.board)..replaceRange(
    ...
    for (var i = 0; i < word.length; i++) {
      ...
      if (state.secretWord[i] != word[i]) {
        if (secretWordDictionary.containsKey(state.secretWord[i])) {
          secretWordDictionary[state.secretWord[i]] =
    ...
    for (final e in resultWord) {
      if (!newStatuses.containsKey(e.letter) ||
          newStatuses.containsKey(e.letter) &&
