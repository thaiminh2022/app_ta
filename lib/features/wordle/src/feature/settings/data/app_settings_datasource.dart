...
import 'package:app_ta/features/wordle/src/core/utils/color_codec.dart';
import 'package:app_ta/features/wordle/src/core/utils/persisted_entry.dart';
import 'package:app_ta/features/wordle/src/feature/settings/data/color_mode_codec.dart';
import 'package:app_ta/features/wordle/src/feature/settings/data/theme_mode_codec.dart';
import 'package:app_ta/features/wordle/src/feature/settings/model/app_settings.dart';
...
class AppSettingsPersistedEntry extends SharedPreferencesEntry<AppSettings> {
  ...
  Future<AppSettings?> read() async {
    ...
    return AppSettings(
      appTheme:
          appTheme ??
          AppTheme(themeMode: ThemeMode.system, colorMode: ColorMode.casual),
  ...
  Future<void> remove() async {
    await [
      ...
      for (final themeMode in ThemeMode.values)
        _otherColor1(themeMode.index).remove(),
      for (final themeMode in ThemeMode.values)
        _otherColor2(themeMode.index).remove(),
      for (final themeMode in ThemeMode.values)
        _otherColor3(themeMode.index).remove(),
    ].wait;
  }
  ...
  Future<void> set(AppSettings value) async {
    await [
      ...
      if (value.appTheme.otherColors != null) ...[
        _otherColor1(
          value.appTheme.themeMode.index,
        ).set(_colorCodec.encode(value.appTheme.otherColors!.$1)),
        _otherColor2(
          value.appTheme.themeMode.index,
        ).set(_colorCodec.encode(value.appTheme.otherColors!.$2)),
        _otherColor3(
          value.appTheme.themeMode.index,
        ).set(_colorCodec.encode(value.appTheme.otherColors!.$3)),
      ],
    ].wait;
