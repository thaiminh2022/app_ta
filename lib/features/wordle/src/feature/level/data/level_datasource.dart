...
final class LevelDatasource implements ILevelDatasource {
  ...
  IntPreferencesEntry get _levelsMigrationVersion => IntPreferencesEntry(
    sharedPreferences: sharedPreferences,
    key: 'level.migration.version',
  );
  ...
  Future<List<GameResult>?> getLevels(String dictionaryKey) async {
    ...
    try {
      return levels
          .map(
            (l) => GameResult.fromJson(json.decode(l) as Map<String, dynamic>),
          )
          .toList();
    } on Object catch (_) {
  ...
  Future<void> setLevel(String dictionaryKey, GameResult level) async {
    ...
    final rawLevels = previousLevels
  ...
  Future<void> runMigration() async {
    ...
    for (final dictionary in dictionaries) {
      try {
        ...
        if (!decodedFirst.containsKey('word') ||
            !decodedFirst.containsKey('isWin')) {
          continue;
        }
        ...
        await _levels(dictionary.languageCode).set(
          newLevels.map((e) => json.encode(e.toJson())).toList(growable: false),
        );
      } on Object {
        // nothing
      }
